...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gallery_action'])) {
    ...
    if (!isset($_POST['csrf_token']) || !isset($_SESSION['csrf_token'])
        ...
        // Direct skin-package UPLOAD removed (Sean, 2026-08-03): closed beta uses
        // registry skins only; custom-skin uploads are a support headache we don't
        // want. Registry install / update / remove / activate remain below.
...
if (isset($_POST['save_skin_settings'])) {
    ...
    foreach ($manifest['options'] as $key => $meta) {
        ...
        if ($meta['type'] === 'select' && isset($meta['options'][$val]['css'])) {
            ...
            // Companion font-size is optional. Skins with their own responsive
            // typography control can opt out so this generic rem value does not
            // silently override their sizing system.
...
<?php elseif ($active_tab === 'gallery'): ?>
    ...
    ?>
    <?php // Direct skin-package UPLOAD UI removed 2026-08-03 (Sean): closed beta
          // uses registry skins only — no custom-skin uploads (support headache).
