/**
 * SNAPSMACK - AURORA Post Modal
 *
 * Intercepts .au-tile link clicks and renders the post as an IG-style overlay
 * instead of navigating to a new page. Fetches the post URL with &modal=1
 * appended, which instructs layout.php to return only the inner .au-post-ig
...
(function () {
    ...
    var overlay  = document.getElementById('au-modal-overlay');
    var frame    = document.getElementById('au-modal-frame');
    var backdrop = overlay ? overlay.querySelector('.au-modal-backdrop') : null;
    ...
    if (!overlay || !frame) {
        ...
        console.warn('[au-modal] #au-modal-overlay / #au-modal-frame not in DOM — ' +
    ...
    function initCarouselIn(root, _attempt) {
        if (typeof SnapSlider === 'undefined') {
            ...
            _attempt = _attempt || 0;
            if (_attempt < 10) {
                console.warn('[au-modal] SnapSlider not ready — retry ' + (_attempt + 1) + '/10');
                setTimeout(function () { initCarouselIn(root, _attempt + 1); }, 150);
            } else {
                console.warn('[au-modal] SnapSlider never loaded — carousel nav disabled.');
            }
            return;
        }
        ...
        for (var i = 0; i < sliders.length; i++) {
            ...
            try {
                ...
                console.warn('[au-modal] SnapSlider init failed:', err);
                continue;
            }
    ...
    function wireExifPanel(container, root) {
        container.addEventListener('snapslider:slidechange', function (e) {
            ...
            var panel = root.querySelector('#au-exif-panel');
            if (!panel) return;
            panel.innerHTML = '';
            var fields = {
            ...
            Object.keys(fields).forEach(function (key) {
                ...
                item.className = 'au-exif-item';
                item.setAttribute('data-exif-key', key);
                var lbl = document.createElement('span');
                lbl.className = 'au-exif-label';
                lbl.textContent = fields[key];
                var valEl = document.createElement('span');
                valEl.className = 'au-exif-value';
    ...
    document.addEventListener('click', function (e) {
        var tile = e.target.closest('.au-tile a');
        if (!tile) return;
        e.preventDefault();
        openModal(tile.href);
    });

    // ── In-modal back arrow (.au-back-btn) → close cleanly ───────────────────
    document.addEventListener('click', function (e) {
        if (e.target.closest('.au-back-btn')) {
            e.preventDefault();
            closeModal();
        }
    });
