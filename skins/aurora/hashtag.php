<?php
/**
 * SNAPSMACK - AURORA Hashtag Archive
 * Alpha v0.7.9
 *
...
$hover_overlay = $settings['au_hover_overlay']      ?? 'dark';
...
<div id="au-app">
    ...
    <div class="au-archive-header">
        <a href="<?php echo BASE_URL; ?>" class="au-back-link">
    ...
    <!-- ── Grid ────────────────────────────────────────────────────────────── -->
    <main class="au-grid" aria-label="<?php echo htmlspecialchars($tag_display); ?> photos">
...
<script>
(function () {
    var sentinel = document.getElementById('au-sentinel');
    ...
    var obs = new IntersectionObserver(function (entries) {
        ...
        fetch(url)
            ...
            .then(function (html) {
                ...
                var newTiles = tmp.querySelectorAll('.au-grid .au-tile');
                newTiles.forEach(function (t) { grid.appendChild(t); });

                // Check if there are more pages
                var newSentinel = tmp.querySelector('#au-sentinel');
                if (newSentinel) {
                    nextPg++;
                    loading = false;
                } else {
