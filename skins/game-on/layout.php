...
// go-modal.js (via skin-footer's data-autoopen) to open this post's modal over
// the grid — Instagram-style deep linking. The .go-post-ig markup below is
...
$_go_customize_level = $settings['go_customize_level'] ?? 'per_grid';

$go_resolve_frame = function ($pi_row) use ($settings, $_go_customize_level, $_go_shadow_map, &$post) {
    ...
    $crop = 'fit';
    if ($_go_customize_level === 'per_image' || $has_per_image) {
        $crop = $pi_crop;
        $sz  = $pi_sz;
        $bpx = $pi_bpx;
        ...
        $sh  = $pi_sh;
    } elseif ($_go_customize_level === 'per_carousel') {
...
$_go_modal_mode = !empty($_GET['modal']);
...
?>
<div class="go-post-ig">

    <!-- ── LEFT: image panel (never scrolls) ───────────────────────────── -->
    <div class="go-post-ig-image">

        <?php if ($is_carousel): ?>
        <!-- Carousel -->
        <div class="go-carousel-wrap">
            <div id="go-carousel"
                 class="ss-slider"
                ...
                <div class="slider-track">
                    <?php foreach ($post_images as $pimg):
                        ...
                        if ($frame['is_framed']):
                            ...
                            $slide_class .= ' go-slide--framed';
                        elseif ($frame['is_fill']):
                            $slide_class .= ' go-crop--fill';   // IG square crop
                        endif;
                    ?>
        ...
        <?php else:
            $frame = $go_resolve_frame($cover_img);
    ...
    <div class="go-post-ig-info">

        <!-- Fixed header -->
        <div class="go-post-ig-header">
            <button class="go-back-btn" type="button" aria-label="Back to grid">&#8592;</button>
            <?php if ($_avatar_exists): ?>
                <img class="go-post-ig-avatar"
                     src="<?php echo BASE_URL . htmlspecialchars($_avatar_path); ?>"
                     alt="">
            <?php else: ?>
                <span class="go-post-ig-avatar go-post-ig-avatar--initials"><?php echo htmlspecialchars($_avatar_initial); ?></span>
            <?php endif; ?>
            <span class="go-post-ig-sitename"><?php echo htmlspecialchars($_site_name); ?></span>
        </div>

        <!-- Scrollable body -->
        <div class="go-post-ig-body">

            <!-- Caption IG-style: bold sitename inline + caption text -->
            <div class="go-post-caption-block">
                <p class="go-post-ig-caption">
                    <span class="go-post-ig-caption-user"><?php echo htmlspecialchars($_site_name); ?></span>
                    <?php
                    $caption_parts = [];
                    if (!empty($post['title'])) {
                    ...
                    if (!empty($post['description'])) {
                        $desc_html = snap_render_caption_html($post['description'], BASE_URL, 'go-hashtag');
            ...
            foreach ($exif_fields as $key => $label):
                ...
                if ($val === '') continue;
                $exif_rows .= '<div class="go-exif-item" data-exif-key="' . $key . '">'
                            . '<span class="go-exif-label">' . $label . '</span>'
                            . '<span class="go-exif-value">' . htmlspecialchars($val) . '</span></div>';
            endforeach;
            $exif_any = ($exif_rows !== '') || !empty(array_filter($exif_map));
            if ($exif_any):
            ?>
            <!-- EXIF panel — updated by snapslider:slidechange in carousel mode -->
            <div id="go-exif-panel" class="go-exif-panel"><?php echo $exif_rows; ?></div>
            <?php endif; ?>

            <?php if (!empty($post['allow_comments'])): ?>
            <div class="go-community-wrap">
                <?php
                $img = $cover_img;
        ...
        <div class="go-post-ig-actions">
            <div class="go-post-ig-action-icons">
                <button type="button" class="go-action-btn" aria-label="Comment" data-ss-action="scroll-to" data-ss-target=".go-community-wrap">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                </button>
                <button class="go-action-btn go-action-bookmark" aria-label="Save">
            ...
            <p class="go-post-ig-date"><?php echo date('F j, Y', strtotime($post['created_at'])); ?></p>
        </div>

        <?php if (!$_go_modal_mode) include __DIR__ . '/skin-footer.php'; ?>
