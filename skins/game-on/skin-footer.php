...
?>
<!-- Post modal overlay (populated by go-modal.js). Rendered in the SHARED
...
<div id="go-modal-overlay" class="go-modal-overlay" hidden
     data-grid-url="<?php echo htmlspecialchars(BASE_URL); ?>"<?php echo !empty($go_autoopen) ? ' data-autoopen="1"' : ''; ?>>
    <div class="go-modal-backdrop"></div>
    <div id="go-modal-frame" class="go-modal-frame"></div>
</div>

<!-- Avatar lightbox (populated by go-lightbox.js). Shared by every Grid page. -->
<div id="go-lightbox" class="go-lightbox" hidden>
    <button type="button" class="go-lightbox-close" aria-label="Close">&times;</button>
    <img class="go-lightbox-img" src="" alt="Profile photo">
</div>
<?php
