<?php
/**
 * SNAPSMACK - JIVE TURKEY Help Topics
 * Alpha v0.7.9
 *
 * Returns help content for the JIVE TURKEY skin.
...
return [
    'skin_name' => 'JIVE TURKEY',
    'topics' => [
        [
            'title' => 'Three-Column Grid Layout',
            'body'  => 'JIVE TURKEY presents your photographs in a clean, square-tile 3-column layout — the same format that made Instagram the de-facto portfolio platform for photographers. Every post occupies one tile on the landing page, regardless of how many images it contains. Tap or click any tile to view the full post.',
        ],
        [
            'title' => 'The Jive Turkey Background',
            'body'  => 'Behind your photographs, slow northern-lights curtains drift across the page — drawn on a canvas, advancing through the active palette over a cycle you choose (15–240 seconds; the default is deliberately geological). It is meant to be felt, not watched. In Skin Admin under JIVE TURKEY you set the palette (Jive Turkey, Borealis Ice, or Solar Storm), the sky base colour (deep black, navy, or indigo), the cycle time, and the background opacity. The colour shift is palette-driven — the real palette colours always show true, so nothing washes out or shifts off-hue.',
        ],
        [
            'title' => 'The Border Wave',
            'body'  => 'Each tile carries an animated colour border drawn from the JIVE TURKEY palette. Pick the style in Skin Admin under BORDER WAVE: Circle each tile, Circle + sweep across, Wave across grid, or Scatter pulse. You also control the direction (including the two diagonals), the wave rhythm (a slow–fast–slow breath, or constant slow), the border width (1–10 px), the border opacity, and the tile corners. The border shares the JIVE TURKEY palette, so changing the palette restyles both the background and the borders at once.',
        ],
        [
            'title' => 'Motion & Accessibility',
            'body'  => 'Both layers respect your operating system\'s "reduce motion" setting. When reduced motion is on, the jive-turkey background and the tile borders each render a single static frame — the skin stays visually coherent with zero movement. The animation also pauses automatically whenever the browser tab is hidden, so it never burns CPU in the background.',
        ],
        [
        ...
        [
            ...
            'body'  => 'JIVE TURKEY lets you composite your photographs into custom frames — the kind of careful presentation photographers used to do in Photoshop actions or Instasize before the platforms stripped it out. In Skin Admin under IMAGE FRAME, set the customisation mode: Per Grid applies one style to every tile; Per Carousel lets each post define its own look; Per Image gives every individual photo its own treatment. Style controls per image: size within the square (75–100% in 5% increments), border width (0–20 px), border colour, background colour, and drop shadow intensity (none, soft, medium, heavy). When a frame is applied, the tile switches from full-bleed cover crop to a flex-centred contained image over a coloured background.',
        ],
        [
