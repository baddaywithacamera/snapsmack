...
return [
    'name'        => 'JIVE TURKEY',
    'version'     => '0.1.23', // 0.1.23: photo back INSIDE the ring (inset --tile-bw) at r=--tile-radius, concentric with the 0.1.22 outer radius; pairs with the 0.7.433 engine that pins the band to the photo edge (shrink collapses INTO the photo, background floods behind). // 0.1.22: tile outer corner = --tile-radius + --tile-bw so the colour band is a uniform concentric frame (no corner bulge/peek at any pulse width). // 0.1.21: photo fills the tile (clipped to full radius) so corners stay round through the pulse; band rides the outer edge; engine back to 429 padding-pulse. // 0.1.20: border SHRINK/EXPAND width-pulse restored via .jt-ring padding (inward; never touches 20px Tile Spacing or the photo); grid left/right band-reserve padding dropped so outer tiles align with nav. // 0.1.19: photo sits INSIDE the border ring again (inset by ring width); 20px Tile Spacing is pure background between bordered tiles; images shrink so border+gaps fit the 935px column (undoes 0.1.17/0.7.425 outward-band). // 0.1.18: SCROLLS travels in all 8 directions (up/down/left/right + 4 diagonals, or random over 8). // 0.1.17: JIVE TURKEY border restored to OUTWARD box-shadow band (outside the photo, image never resizes) with the tile spacing that worked; image corner now matches the band corner (no corner gap); new Transition Length control. // 0.1.16: SCROLLS background mode (recoloured 70s ribbon-scroll tile; direction v/h/diag/random; colour drift fade/blink/off) + tile corner radius nests inside the ring. 0.1.15: SOLO IMAGE presentation — backdrop (skin|image) + scrim (colour@opacity) + card (colour/padding/text primary+secondary); supersedes 0.1.14 scrim-only; 0.1.4: border rides .jt-ring (no photo resize / dark corners); DAISY clears panel; crisp bg, % units, regrouped settings
