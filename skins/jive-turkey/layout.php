<?php
/**
 * SNAPSMACK - JIVE TURKEY Post View
 * Alpha v0.7.9
 *
...
// jt-modal.js (via skin-footer's data-autoopen) to open this post's modal over
// the grid — Instagram-style deep linking. The .jt-post-ig markup below is
...
$_jt_customize_level = $settings['jt_customize_level'] ?? 'per_grid';

$jt_resolve_frame = function ($pi_row) use ($settings, $_jt_customize_level, $_jt_shadow_map, &$post) {
    switch ($_jt_customize_level) {
        case 'per_image':
        ...
        case 'per_carousel':
            ...
            break;
        default: // per_grid
            $sz  = (int)($settings['jt_frame_size_pct']     ?? 100);
            $bpx = (int)($settings['jt_frame_border_px']    ?? 0);
            $bc  = $settings['jt_frame_border_color'] ?? '#000000';
            $bg  = $settings['jt_frame_bg_color']     ?? '#ffffff';
            $sh  = (string)($settings['jt_frame_shadow']    ?? '0');
    }
    return [
        'size_pct'    => $sz,
        'border_px'   => $bpx,
        'border_color'=> $bc,
        'bg_color'    => $bg,
        'shadow_css'  => $_jt_shadow_map[$sh] ?? 'none',
...
$_jt_modal_mode = !empty($_GET['modal']);
...
?>
<div class="jt-post-ig">

    <!-- ── LEFT: image panel (never scrolls) ───────────────────────────── -->
    <div class="jt-post-ig-image">
                ...
                <div class="slider-track">
                    <?php foreach ($post_images as $pimg):
                        ...
                        if ($frame['is_framed']):
                            ...
                            $slide_class = 'slider-slide jt-slide--framed';
                            $img_style   = '';
                        else:
                            $slide_vars  = '';
        ...
        <?php else:
            $frame = $jt_resolve_frame($cover_img);
    ...
    </div><!-- .jt-post-ig-image -->

    <!-- ── RIGHT: info panel (scrollable) ──────────────────────────────── -->
    <div class="jt-post-ig-info">

        <!-- Fixed header -->
        <div class="jt-post-ig-header">
            <button class="jt-back-btn" type="button" aria-label="Back to grid">&#8592;</button>
            <?php if ($_avatar_exists): ?>
                <img class="jt-post-ig-avatar"
                     src="<?php echo BASE_URL . htmlspecialchars($_avatar_path); ?>"
                     alt="">
            <?php else: ?>
                <span class="jt-post-ig-avatar jt-post-ig-avatar--initials"><?php echo htmlspecialchars($_avatar_initial); ?></span>
            <?php endif; ?>
            <span class="jt-post-ig-sitename"><?php echo htmlspecialchars($_site_name); ?></span>
        </div>
        ...
        <div class="jt-post-ig-body">
            ...
            <div class="jt-post-caption-block">
                <p class="jt-post-ig-caption">
                    ...
                    if (!empty($post['description'])) {
                        $desc_html = snap_render_caption_html($post['description'], BASE_URL, 'jt-hashtag');
            ...
            foreach ($exif_fields as $key => $label):
                ...
                if ($val === '') continue;
                $exif_rows .= '<div class="jt-exif-item" data-exif-key="' . $key . '">'
                            . '<span class="jt-exif-label">' . $label . '</span>'
                            . '<span class="jt-exif-value">' . htmlspecialchars($val) . '</span></div>';
            endforeach;
            $exif_any = ($exif_rows !== '') || !empty(array_filter($exif_map));
            if ($exif_any):
            ?>
            <!-- EXIF panel — updated by snapslider:slidechange in carousel mode -->
            <div id="jt-exif-panel" class="jt-exif-panel"><?php echo $exif_rows; ?></div>
            <?php endif; ?>

            <?php if (!empty($post['allow_comments'])): ?>
            <div class="jt-community-wrap">
                <?php
                $img = $cover_img;
        ...
        <div class="jt-post-ig-actions">
            <div class="jt-post-ig-action-icons">
                <button class="jt-action-btn" aria-label="Comment" onclick="document.querySelector('.jt-community-wrap')?.scrollIntoView({behavior:'smooth'})">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                </button>
                <button class="jt-action-btn jt-action-bookmark" aria-label="Save">
            ...
            <p class="jt-post-ig-date"><?php echo date('F j, Y', strtotime($post['created_at'])); ?></p>
        </div>

        <?php if (!$_jt_modal_mode) include __DIR__ . '/skin-footer.php'; ?>
