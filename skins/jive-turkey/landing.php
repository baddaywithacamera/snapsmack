<?php
/**
 * SNAPSMACK - JIVE TURKEY Landing Page
 *
...
$customize_level = $settings['jt_customize_level']     ?? 'per_grid';
...
$jt_resolve_tile_frame = function ($cover_pi_row, $post_row) use ($settings, $customize_level, $_jt_shadow_map) {
...
?>
<div class="jt-content-wrap landing-feed">
...
<main>
    <div class="jt-grid">
        ...
        $slot_class_h = [1 => 'jt-tile--trigram-L', 2 => 'jt-tile--trigram-M', 3 => 'jt-tile--trigram-R'];
        $slot_class_v = [1 => 'jt-tile--trigram-T', 2 => 'jt-tile--trigram-M', 3 => 'jt-tile--trigram-B'];

        $col = 0; // track current column position (0, 1, 2)
        $jt_idx = 0; // running cell index (incl. phantoms) → data-row/data-col for the wave

        foreach ($grid_posts as $post):
            $jt_slot   = (int)($post['trigram_slot'] ?? 0);
            $jt_orient = $post['trigram_orientation'] ?? 'h';
            $jt_id     = (int)($post['trigram_id'] ?? 0);

            // ── Phantom padding ──────────────────────────────────────────
            // When the L post (slot 1) of a horizontal trigram falls off the
            // start of a row, emit invisible phantom tiles to complete the
            // current row first.
            if ($jt_slot === 1 && $jt_orient !== 'v' && $col !== 0):
                $phantoms = 3 - $col;
                for ($ph = 0; $ph < $phantoms; $ph++):
        ?>
        <div class="jt-tile jt-tile--phantom" aria-hidden="true"
             data-row="<?php echo intdiv($jt_idx, 3); ?>" data-col="<?php echo $jt_idx % 3; ?>"></div>
        <?php
                    $col = ($col + 1) % 3;
                    $jt_idx++;
                endfor;
            ...
            if ($jt_id > 0 && $jt_slot > 0) {
                $jt_label = ($jt_orient === 'v')
                    ? (['T','M','B'][$jt_slot - 1] ?? '')
                    : (['L','M','R'][$jt_slot - 1] ?? '');
                if ($jt_label !== '') {
                    $jt_rel = 'trigrams/trigram-' . $jt_id . '-' . $jt_label . '.jpg';
                    if (is_file(dirname(__DIR__, 2) . '/' . $jt_rel)) {
                        $thumb_src = $jt_rel;
                        $is_slice_tile = true;
                    }
                }
            ...
            // ── Tile class ───────────────────────────────────────────────
            $tile_frame = $jt_resolve_tile_frame($post, $post);
            $tile_class = 'jt-tile';

            if ($jt_id > 0 && $jt_slot > 0) {
                $sc = ($jt_orient === 'v') ? ($slot_class_v[$jt_slot] ?? '') : ($slot_class_h[$jt_slot] ?? '');
                if ($sc) $tile_class .= ' ' . $sc;
                $tile_class .= ' jt-tile--trigram';
            }
            ...
            if ($do_frame) {
                ...
                $tile_class .= ' jt-tile--framed';
                if ((int)$post['img_height'] > (int)$post['img_width']) {
                    $tile_class .= ' jt-tile--portrait';
                }
            }
        ...
        <div class="<?php echo $tile_class; ?>"
             data-trigram-id="<?php echo $jt_id; ?>"
             data-trigram-slot="<?php echo $jt_slot; ?>"
...
<?php /* Post modal overlay is now rendered once by skin-footer.php (shared by all
         Grid pages) so jt-modal.js finds its container on every page, not just
