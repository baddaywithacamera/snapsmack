 ...
 * builder, same toggle pattern, same inline JS. Class names use pfd- prefix.
 *
...
$_pfd_cur = isset($archive_layout) ? $archive_layout : ($settings['archive_layout'] ?? 'thumbs');
$_pfd_initial_thumbs = ($_pfd_cur === 'thumbs');
?>

<!-- Cropped grid — natural aspect ratio thumbnails -->
<div id="browse-grid" class="pfd-archive-grid archive-grid" <?php echo $_pfd_initial_thumbs ? '' : 'style="display:none;"'; ?>>
    <?php if (!empty($images)): ?>
        ...
        ?>
            <a href="<?php echo $link; ?>" class="pfd-archive-item" title="<?php echo htmlspecialchars($img['img_title'] ?? ''); ?>">
                <div class="pfd-thumb<?php echo $ratio < 1 ? ' pfd-thumb-portrait' : ''; ?>" style="aspect-ratio: <?php echo round($ratio, 4); ?>;">
                    <img src="<?php echo $thumb_url; ?>"
                         alt="<?php echo htmlspecialchars($img['img_title'] ?? ''); ?>"
                         loading="lazy">
                </div>
                <div class="pfd-archive-title"><?php echo htmlspecialchars($img['img_title'] ?? ''); ?></div>
            </a>
        <?php endforeach; ?>
    <?php else: ?>
...
<div id="justified-grid" style="--justified-gap: <?php echo $gap; ?>px; --justified-row-height: <?php echo $target_row_h; ?>px; --last-row-ar-sum: <?php echo round($last_full_ar_sum, 4); ?>; <?php echo $_pfd_initial_thumbs ? 'display:none;' : ''; ?>">
    <?php if (!empty($images)): ?>
