...
// he-modal.js (via skin-footer's data-autoopen) to open this post's modal over
// the grid — Instagram-style deep linking. The .he-post-ig markup below is
...
$_he_customize_level = $settings['he_customize_level'] ?? 'per_grid';

$he_resolve_frame = function ($pi_row) use ($settings, $_he_customize_level, $_he_shadow_map, &$post) {
    switch ($_he_customize_level) {
        case 'per_image':
...
$_he_modal_mode = !empty($_GET['modal']);
...
?>
<div class="he-post-ig">

    <!-- ── LEFT: image panel (never scrolls) ───────────────────────────── -->
    <div class="he-post-ig-image">

        <?php if ($is_carousel): ?>
        <!-- Carousel -->
        <div class="he-carousel-wrap">
            <div id="he-carousel"
                 class="ss-slider"
                ...
                <div class="slider-track">
                    <?php foreach ($post_images as $pimg):
                        ...
                        if ($frame['is_framed']):
                            ...
                            $slide_class = 'slider-slide he-slide--framed';
                            $img_style   = '';
                        else:
                            $slide_vars  = '';
        ...
        <?php else:
            $frame = $he_resolve_frame($cover_img);
    ...
    </div><!-- .he-post-ig-image -->

    <!-- ── RIGHT: info panel (scrollable) ──────────────────────────────── -->
    <div class="he-post-ig-info">

        <!-- Fixed header -->
        <div class="he-post-ig-header">
            <button class="he-back-btn" type="button" aria-label="Back to grid">&#8592;</button>
            <?php if ($_avatar_exists): ?>
                <img class="he-post-ig-avatar"
                     src="<?php echo BASE_URL . htmlspecialchars($_avatar_path); ?>"
                     alt="">
            <?php else: ?>
                <span class="he-post-ig-avatar he-post-ig-avatar--initials"><?php echo htmlspecialchars($_avatar_initial); ?></span>
            <?php endif; ?>
            <span class="he-post-ig-sitename"><?php echo htmlspecialchars($_site_name); ?></span>
        </div>

        <!-- Scrollable body -->
        <div class="he-post-ig-body">

            <!-- Caption IG-style: bold sitename inline + caption text -->
            <div class="he-post-caption-block">
                <p class="he-post-ig-caption">
                    <span class="he-post-ig-caption-user"><?php echo htmlspecialchars($_site_name); ?></span>
                    <?php
                    $caption_parts = [];
                    if (!empty($post['title'])) {
                    ...
                    if (!empty($post['description'])) {
                        $desc_html = snap_render_caption_html($post['description'], BASE_URL, 'he-hashtag');
            ...
            foreach ($exif_fields as $key => $label):
                ...
                if ($val === '') continue;
                $exif_rows .= '<div class="he-exif-item" data-exif-key="' . $key . '">'
                            . '<span class="he-exif-label">' . $label . '</span>'
                            . '<span class="he-exif-value">' . htmlspecialchars($val) . '</span></div>';
            endforeach;
            $exif_any = ($exif_rows !== '') || !empty(array_filter($exif_map));
            if ($exif_any):
            ?>
            <!-- EXIF panel — updated by snapslider:slidechange in carousel mode -->
            <div id="he-exif-panel" class="he-exif-panel"><?php echo $exif_rows; ?></div>
            <?php endif; ?>

            <?php if (!empty($post['allow_comments'])): ?>
            <div class="he-community-wrap">
                <?php
                $img = $cover_img;
        ...
        <div class="he-post-ig-actions">
            ...
            <p class="he-post-ig-date"><?php echo date('F j, Y', strtotime($post['created_at'])); ?></p>
        </div>

        <?php if (!$_he_modal_mode) include __DIR__ . '/skin-footer.php'; ?>
