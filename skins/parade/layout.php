...
// pa-modal.js (via skin-footer's data-autoopen) to open this post's modal over
// the grid — Instagram-style deep linking. The .pa-post-ig markup below is
...
$_pa_customize_level = $settings['pa_customize_level'] ?? 'per_grid';

$pa_resolve_frame = function ($pi_row) use ($settings, $_pa_customize_level, $_pa_shadow_map, &$post) {
    switch ($_pa_customize_level) {
        case 'per_image':
...
$_pa_modal_mode = !empty($_GET['modal']);
...
?>
<div class="pa-post-ig">

    <!-- ── LEFT: image panel (never scrolls) ───────────────────────────── -->
    <div class="pa-post-ig-image">

        <?php if ($is_carousel): ?>
        <!-- Carousel -->
        <div class="pa-carousel-wrap">
            <div id="pa-carousel"
                 class="ss-slider"
                ...
                <div class="slider-track">
                    <?php foreach ($post_images as $pimg):
                        ...
                        if ($frame['is_framed']):
                            ...
                            $slide_class = 'slider-slide pa-slide--framed';
                            $img_style   = '';
                        else:
                            $slide_vars  = '';
        ...
        <?php else:
            $frame = $pa_resolve_frame($cover_img);
    ...
    </div><!-- .pa-post-ig-image -->

    <!-- ── RIGHT: info panel (scrollable) ──────────────────────────────── -->
    <div class="pa-post-ig-info">

        <!-- Fixed header -->
        <div class="pa-post-ig-header">
            <button class="pa-back-btn" type="button" aria-label="Back to grid">&#8592;</button>
            <?php if ($_avatar_exists): ?>
                <img class="pa-post-ig-avatar"
                     src="<?php echo BASE_URL . htmlspecialchars($_avatar_path); ?>"
                     alt="">
            <?php else: ?>
                <span class="pa-post-ig-avatar pa-post-ig-avatar--initials"><?php echo htmlspecialchars($_avatar_initial); ?></span>
            <?php endif; ?>
            <span class="pa-post-ig-sitename"><?php echo htmlspecialchars($_site_name); ?></span>
        </div>

        <!-- Scrollable body -->
        <div class="pa-post-ig-body">

            <!-- Caption IG-style: bold sitename inline + caption text -->
            <div class="pa-post-caption-block">
                <p class="pa-post-ig-caption">
                    <span class="pa-post-ig-caption-user"><?php echo htmlspecialchars($_site_name); ?></span>
                    <?php
                    $caption_parts = [];
                    if (!empty($post['title'])) {
                    ...
                    if (!empty($post['description'])) {
                        $desc_html = snap_render_caption_html($post['description'], BASE_URL, 'pa-hashtag');
        ...
        <div class="pa-post-ig-actions">
            <div class="pa-post-ig-action-icons">
                <button class="pa-action-btn" aria-label="Comment" onclick="document.querySelector('.pa-community-wrap')?.scrollIntoView({behavior:'smooth'})">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                </button>
                <button class="pa-action-btn pa-action-bookmark" aria-label="Save">
            ...
            <p class="pa-post-ig-date"><?php echo date('F j, Y', strtotime($post['created_at'])); ?></p>
        </div>

        <?php if (!$_pa_modal_mode) include __DIR__ . '/skin-footer.php'; ?>
