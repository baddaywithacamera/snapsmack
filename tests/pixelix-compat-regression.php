...
$checks = [
    ...
    'status deletion'          => "method==='DELETE'",
    'delete ownership check'   => "This Pixelix authorization does not own that post",
    'delete federation retract'=> "sv_retract_note",
    'delete complete post'     => "DELETE FROM snap_posts WHERE id=?",
    ...
    'published ALT edit'       => "i.post_id IS NOT NULL OR pi.post_id IS NOT NULL OR om.image_id IS NOT NULL",
    'unchanged ALT not false 404'=> "existence separately because an unchanged value",
    'status edit endpoint'     => "statuses/(\\d+)$#',\$route,\$m) && \$method==='PUT'",
    'status edit updates post' => "UPDATE snap_posts SET description=?,allow_comments=?,is_sensitive=?,content_warning=?,updated_at=NOW()",
    'status edit updates images'=> "UPDATE snap_images i JOIN snap_post_images pi ON pi.image_id=i.id SET i.img_description=?,i.allow_comments=? WHERE pi.post_id=?",
    'inline edited ALT text'   => "media_attributes",
    'status edit federates'    => "sv_federate_post_change(\$pdo,\$px_settings,\$pid)",
