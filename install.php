...
';
    ...
    if (empty($errors)) {
        ...
        // Generate root .htaccess if one doesn't already exist
        // (don't overwrite — the user may have customised it)
        if (!file_exists(__DIR__ . '/.htaccess')) {
            $root_htaccess = <<<'HTACCESS'
# ─────────────────────────────────────────────────────────────
# SNAPSMACK — Root .htaccess (generated by installer)
# ─────────────────────────────────────────────────────────────
...
HTACCESS;
            @file_put_contents(__DIR__ . '/.htaccess', $root_htaccess);
        }
    }
