            ...
            try {
                ...
                if ($_SESSION['db_attempts'] >= 5) {
                    ...
                    if (strpos($msg, 'Access denied') !== false) {
                        ...
                        $errors[] = 'Connection failed. Check your credentials and hostname, then try again.';
                    }
                }
                $step = 2;
            }
...
if ($step === 3 && $_SERVER['REQUEST_METHOD'] === 'POST' && empty($errors)) {
    ...
    if (empty($errors)) {
        ...
        $_dead_tables = [
            ...
            // --- BLOGROLL CATEGORIES ---
            "{$prefix}blogroll_cats" => "CREATE TABLE IF NOT EXISTS `{$prefix}blogroll_cats` (
                `id`       int         NOT NULL AUTO_INCREMENT,
                `cat_name` varchar(100) NOT NULL,
                PRIMARY KEY (`id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
            ...
            // --- STATS (raw hit log) ---
            "{$prefix}stats" => "CREATE TABLE IF NOT EXISTS `{$prefix}stats` (
                `id`            int unsigned  NOT NULL AUTO_INCREMENT,
                `hit_at`        datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP,
                `page_type`     varchar(50)   NOT NULL DEFAULT 'unknown',
                `page_slug`     varchar(600)  DEFAULT NULL,
            ...
            // --- STATS DAILY (rolled-up aggregates) ---
            "{$prefix}stats_daily" => "CREATE TABLE IF NOT EXISTS `{$prefix}stats_daily` (
                `id`              int unsigned NOT NULL AUTO_INCREMENT,
                ...
                `top_referrer`    varchar(255) DEFAULT NULL,
                PRIMARY KEY (`id`),
                UNIQUE KEY `uq_stat_date` (`stat_date`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",

            // --- PIMPOTRON SLIDESHOWS ---
            "{$prefix}pimpotron_slideshows" => "CREATE TABLE IF NOT EXISTS `{$prefix}pimpotron_slideshows` (
                `id`                  int unsigned NOT NULL AUTO_INCREMENT,
                `name`                varchar(255) NOT NULL,
                `slug`                varchar(300) NOT NULL,
                `default_speed_ms`    int          NOT NULL DEFAULT 5000,
                `glitch_frequency`    varchar(30)  NOT NULL DEFAULT 'occasional',
                `glitch_intensity`    varchar(30)  NOT NULL DEFAULT 'normal',
                ...
                `slideshow_font`      varchar(100) NOT NULL DEFAULT 'Stalinist One',
            ...
            // --- PIMPOTRON SLIDES ---
            "{$prefix}pimpotron_slides" => "CREATE TABLE IF NOT EXISTS `{$prefix}pimpotron_slides` (
                `id`                   int unsigned NOT NULL AUTO_INCREMENT,
                `slideshow_id`         int unsigned NOT NULL,
                `slide_type`           varchar(30)  NOT NULL DEFAULT 'image',
                `snap_image_id`        int unsigned DEFAULT NULL,
                `external_image_url`   varchar(500) DEFAULT NULL,
...
if ($step === 4 && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['admin_user']) && empty($errors)) {
    ...
    // Safety check: if an admin user already exists this is not a fresh install.
    // Block step 4 entirely to prevent creating a backdoor account that bypasses 2FA.
    try {
        $dsn_check = "mysql:host={$_SESSION['db_host']};dbname={$_SESSION['db_name']};charset=utf8mb4";
        $pdo_check = new PDO($dsn_check, $_SESSION['db_user'], $_SESSION['db_pass'], [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
        $prefix_check = $_SESSION['db_prefix'];
        $existing_admin = $pdo_check->query("SELECT COUNT(*) FROM `{$prefix_check}users` WHERE user_role = 'admin' LIMIT 1")->fetchColumn();
        if ($existing_admin > 0) {
            $errors[] = 'An admin account already exists on this database. The installer cannot create another. Delete install.php and log in normally.';
            $step = 4;
        }
    } catch (PDOException $e) {
        // Table doesn't exist yet — fresh install, safe to continue
    }
...
RewriteRule ^app/?$ smack-app.php [L,QSA]
RewriteRule ^api/desktop-auth/(activate|enroll|status|renew)/?$ api.php?route=desktop-auth/$1 [L,QSA]
RewriteRule ^api/v([12])/(.*)$ pixelfed-api.php?route=api/v$1/$2 [L,QSA]
...
/**
 * SNAPSMACK - Core Database Connection
 * ' . $installer_version_label . ' (generated by recovery)
 */
...
';
                        ...
                        if (!file_exists(__DIR__ . '/core/constants.php')) {
                            $const_php = '<?php
define(\'SNAPSMACK_VERSION\', \'' . $installer_version_label . '\');
define(\'SNAPSMACK_VERSION_SHORT\', \'' . $installer_version . '\');
define(\'SNAPSMACK_VERSION_CODENAME\', \'Shit and Git\');
