...
- Impact Printer: inline `[img:]` images now use the same ASCII border pattern as the hero (style, colour, ink opacity all inherited). Two new PRINT HEAD admin controls — **Inline Image Border Weight** (default 9 px) and **Inline Image Border Padding** (default 8 px) — let the border be sized independently of the hero. Selecting "No Frame" in the hero border picker removes borders from inline images too.
