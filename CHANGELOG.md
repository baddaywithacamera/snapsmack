...
- Skins: Instant Camera 1.0.8 → 1.0.11, AURORA 1.0.21 → 1.0.23, PARADE 1.2.10 → 1.2.12. (The `.11 / .23 / .12` patch — skin-only, shipped after the CMS 0.7.330 core push — moves the navbar Colour+Opacity onto the **stuck/scroll state only** (`.profile-hidden`), not the inline landing nav, and adds a **Landing Panel Extend** control (px, 0–100) so the panel renders as a backing layer behind the tiles and bleeds out past the outer tile edges.)
