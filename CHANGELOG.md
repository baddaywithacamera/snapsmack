...
- **SCROLL header polish (skin 0.1.22).** Dropped the faint hairline under the masthead (the `border-top` on the browse bar), and made the space above the masthead equal the space below it (the profile's top/bottom padding was lopsided). Static pages, the blogroll and photo pages already share the landing's full masthead header via `skin-header.php` — that arrived earlier in this cycle and ships with the skin.
