...
- **Fixed: the public fediverse profile (`instance/username`) showed "0 posts / No public posts yet" with a full library.** The count and photo-grid queries filtered `snap_images` on `is_cover`, but `is_cover` lives on the `snap_post_images` pivot, not `snap_images` — so the query threw and was silently swallowed to zero. It now counts published posts and pulls each post's cover (the `is_cover` pivot row, falling back to the first image) in grid `sort_order`, so the profile grid mirrors the home feed.
