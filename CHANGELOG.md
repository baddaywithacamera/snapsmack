...
- **Older relay installs now receive the missing durable-ingest table.** The canonical schema already defined `snap_relay_ingest_jobs`, but there was no migration for an existing database. 633D adds the migration plus a relay-only `CREATE TABLE IF NOT EXISTS` self-heal immediately before either queue operation; ordinary non-relay blogs still return without touching hub schema.
