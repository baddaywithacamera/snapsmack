...
- **The Skin Gallery can now install a packaged skin even when the registry does not list it.** The owner-only upload path accepts a Skin Packager ZIP, rejects traversal and ambiguous package roots, requires the inert manifest and stylesheet, requires password plus TOTP and explicit executable-code consent, and refuses packages without their SMACKBACK integrity manifest. Registry visibility is no longer a lockout.
