...
- **Hashtag search now uses your piggyback account, so `#tag` actually returns results.** Pixelfed gates its tag timeline behind authentication, so an unauthenticated `#cats` came back empty — the one place that could return a real feed (your configured account's token) was never consulted for hashtags. When a piggyback account is set, hashtag search now runs the tag timeline authenticated on that account's own instance (a large instance = a real federated feed), falling back to the public path only when no account exists. Adds `sv_authed_hashtag_timeline()`; the row-mapping is shared with the public path via the new `sv_map_tag_status()`.
