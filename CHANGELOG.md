...
- **SMACKTALK skin — a TILEZ-masonry variant of the SMACKTALK/ALFRED longform skin.** New skin `skins/smacktalk`, cloned from ALFRED so it reuses ALFRED's proven SmackTalk plumbing verbatim: the `preload.php` router (single post by `?post=`/`?id=`/`?s=`, prev/next, `?view=archive` photo lightbox, and the bare-request feed) and the longform post view. The ONLY change is the look — the feed's `.posts` grid is restyled from ALFRED's square 3-column layout into a dense, varied TILEZ masonry (tall/wide/hero cadence, responsive 4→3→2→1 columns). Same markup, same routing, different index. (`skins/smacktalk/*`.)
- **Install mode 3 now defaults to SMACKTALK.** The installer's per-mode default-skin map sends `smacktalk` → the `smacktalk` skin (was falling through to `new-horizon`). (`install.php`.)
