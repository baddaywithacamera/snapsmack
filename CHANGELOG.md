...
- **SCROLL 0.1.25: solo page now truly matches 50-Shades (no scroll-to-footer), + serif body font for static/blog.** The solo page was viewport-locked but you could still scroll the photo up to reach the footer, unlike 50-Shades. Cause, measured live: `#system-footer` sits *inside* the scrolling stage and was visible, adding exactly 127px (79px height + 48px margin) of overflow. Copied 50-Shades' rule verbatim (scoped to solo): the system footer is hidden until you open the INFO/COMMENTS drawer — then, and only then, does anything scroll. Stage now equals the viewport exactly (header 80 + photo 580 + nav 60 = 720). Also: new **Static & Blog Body Font** control (pick a serif for reading text) while the new **Static & Blog Heading Font** control keeps page titles and blogroll headings sans-serif — two independent controls, matching the 50-Shades split.
- **SCROLL 0.1.24: divider line now lands exactly at the landing's image row on static AND blog pages, + a hero-image spacing control.** The static/blog divider was placed with a wrong 88px offset (a bad harness measurement) and, on the blogroll, core CSS was overriding SCROLL's rule entirely (margin computed to 0). Measured live: header bottom ~155px, landing's first image row at ~195px — so the divider is now a correct **40px** below the shared header on both page types, matching the landing to the pixel. The blogroll rule is scoped to `body.is-blogroll` so it wins over core. New **Hero Image Spacing (Top & Bottom)** control in STATIC & BLOG adds breathing room above/below a static page's hero image (`--scroll-hero-pad`, default 48px). The masthead stays on static/blog pages (as intended) and its controls remain fully separate from the page-title/body controls (verified live).
