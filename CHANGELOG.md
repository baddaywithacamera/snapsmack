...
- Impact Printer: archive thumbnails can now use the same ASCII border patterns as the hero image (box / plus / equals / slash). Select from the **Archive Thumb Frame** picker in PRINT HEAD. A companion **Archive Thumb Border Weight** slider (default 6 px) controls the frame size.
