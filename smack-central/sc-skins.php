...
function sc_read_manifests(string $skins_dir): array {
    $skins = [];
    foreach (glob(rtrim($skins_dir, '/') . '/*/manifest.json') as $mf) {
        $slug = basename(dirname($mf));
        if (!preg_match('/^[a-z0-9][a-z0-9-]*$/', $slug)) continue;
        try {
            $manifest = json_decode((string)file_get_contents($mf), true, 64, JSON_THROW_ON_ERROR);
        } catch (\Throwable $e) {
    ...
    elseif (isset($_POST['package'])) {
        ...
        if (empty($build_errors)) {
            ...
            foreach ($selected as $slug) {
                ...
                // 0.7.440 transition bridge: old core loads manifest.php while
                // new core reads manifest.json. Generate this adapter in the
                // signed ZIP only; it never lives in the source tree.
                $compat_manifest = <<<'PHP'
<?php
/**
 * SnapSmack transitional skin-manifest adapter.
 * Generated by Smack Central; covered by the skin ZIP signature.
 */
$json = @file_get_contents(__DIR__ . '/manifest.json');
$data = is_string($json) ? json_decode($json, true) : null;
return is_array($data) ? $data : [];
PHP;
                $zip->addFromString('manifest.php', $compat_manifest . "\n");
                $file_count++;
                $zip->close();
