...
def export_session(session: Session, dest_dir: str) -> str:
    ...
    with open(os.path.join(out, "RECOVERY.txt"), "w", encoding="utf-8") as f:
        f.write(
            "COLD SNAP — recoverable batch export\n"
            "=========================================\n\n"
            ...
            "  1. Open COLD SNAP -> BATCH, PLEASE (or BATCH SLAPPED).\n"
...
def import_session(export_dir: str, store: SessionStore) -> Session:
    ...
    if not os.path.isfile(mp):
        raise ValueError("Not a COLD SNAP export (no export-manifest.json).")
    with open(mp, "r", encoding="utf-8") as f:
        manifest = json.load(f)
    if int(manifest.get("manifest_version", 0)) > EXPORT_MANIFEST_VERSION:
        raise ValueError(
            "This export was written by a newer build. Update COLD SNAP to import it."
        )
