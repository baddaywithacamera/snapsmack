...
TOPICS = [
    ...
    ("BIGGIE — the WYSIWYG editor",
     ...
     "into), HR (a divider), GAP (a vertical space) and, in COLD TAKE, MOSAIC. IMG opens a "
     "grid of pictures: this post's own photos first (they get their site id when the post "
     "is sent), then the site's Media Gallery as COLD STORAGE has synced it. Click one and "
     "it appears in the page. The gallery half needs one SYNC FROM SITE first; an image id "
     "can still be typed.\n\n"
