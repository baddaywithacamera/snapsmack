...
foreach ($slugs as $slug) {
    ...
    // Transitional rollback bridge for pre-0.7.440 cores. This file exists
    // only inside the package; the source tree remains JSON-only.
    $compat_manifest = <<<'PHP'
<?php
/**
 * SnapSmack transitional skin-manifest adapter.
 * Generated by the packager; production packages are signature-covered.
 */
$json = @file_get_contents(__DIR__ . '/manifest.json');
$data = is_string($json) ? json_decode($json, true) : null;
return is_array($data) ? $data : [];
PHP;
    $zip->addFromString($slug . '/manifest.php', $compat_manifest . "\n");
    $file_count++;
