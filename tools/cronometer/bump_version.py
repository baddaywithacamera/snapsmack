...
"""
CRONOMETER — version bumper.

Called by build.bat before each build (unless `build.bat norev`). Increments
the third component of BUILD_VERSION in cronometer.py and drops a dated CHANGELOG
...
Cloned from COLD SNAP's bump_version.py — only the target filename changes.
"""
...
def read_version() -> "tuple[str, str]":
    ...
    if not m:
        _fail("BUILD_VERSION not found in cronometer.py.")
