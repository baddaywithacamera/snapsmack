...
a = Analysis(
    ...
    datas=[
        ...
        (os.path.join(_src, 'suyb_qt.py'), '.'),
        (os.path.join(_src, 'suyb_qt_common.py'), '.'),
        (os.path.join(_src, 'suyb_qt_audit.py'), '.'),
        (os.path.join(_src, 'suyb_qt_cloudsync.py'), '.'),
        (os.path.join(_src, 'suyb_qt_credentials.py'), '.'),
        (os.path.join(_src, 'suyb_qt_manage.py'), '.'),
        (os.path.join(_src, 'suyb_qt_profiles.py'), '.'),
        (os.path.join(_src, 'suyb_qt_schedule.py'), '.'),
        (os.path.join(_src, 'suyb_qt_settings.py'), '.'),
        (os.path.join(_src, 'suyb_qt_slaphappy.py'), '.'),
        (os.path.join(_src, 'credential_store.py'), '.'),
        (os.path.join(_src, 'os_schedule.py'), '.'),
        (os.path.join(_src, 'headless.py'), '.'),
        (os.path.join(_src, 'ftps_pins.py'), '.'),
...
# Without this the exe carries NO version at all: Properties > Details is blank,
# and there is no way to tell 0.7.45 from 0.7.46 on disk. SUYB had never had one
# (checked 2026-09-23 — the installed exe's FileVersion was empty). Generated
# here from SUYB_BUILD_VER so it can never drift from the exe's name.
_vparts = [int(x) for x in _ver.split('.') if x.isdigit()]
while len(_vparts) < 4:
    _vparts.append(0)
_vt = tuple(_vparts[:4])
_version_file = os.path.join(os.path.dirname(os.path.abspath(SPECPATH)), 'suyb_version_info.txt')
with open(_version_file, 'w', encoding='utf-8') as _fh:
    _fh.write(f"""# Auto-generated by smackupyourbackup.spec — do not edit.
VSVersionInfo(
  ffi=FixedFileInfo(
  ...
  kids=[
    StringFileInfo([
      StringTable('040904B0', [
        ...
        StringStruct('FileDescription', 'SMACK UP YOUR BACKUP - site backup and restore'),
        StringStruct('FileVersion', '{_ver}'),
        StringStruct('InternalName', 'smackupyourbackup'),
        StringStruct('OriginalFilename', 'suyb.exe'),
        StringStruct('ProductName', 'SMACK UP YOUR BACKUP'),
