...
"""
SMACK YOUR MOUTH — version bumper.

Called by build.bat before each build (unless `build.bat norev`). Increments the
third component of BUILD_VERSION in main.py and drops a dated CHANGELOG stub so
the changelog's latest entry keeps matching BUILD_VERSION. Prints the new
version on success.

Pure stdlib. Safe to run by hand; idempotent on the CHANGELOG (won't double-add).
Increments by the SMALLEST step (patch) — never bumps minor/major (family rule).
"""
...
def next_version(ver: str) -> str:
    ...
    if not m:
        _fail(f"version {ver!r} is not plain MAJOR.MINOR.PATCH — bump it by hand.")
