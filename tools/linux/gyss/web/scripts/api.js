...
//
// BLINK PORT NOTE. In the Tauri build these HTTP calls went straight from the
// webview to the blog, because gyss-api.php emits CORS headers for tauri:// and
// file:// origins. The Blink webview's origin is http://127.0.0.1:<port>, which
// gyss-api.php does NOT allow — a browser fetch() would be blocked. So, exactly
// like the Rust build's native thumbnail fetch (download_to), these calls now go
// through the Python side (app.py `http_request`), which is not bound by browser
// CORS. Behaviour and endpoints are unchanged; only the transport moved off the
// browser fetch and onto the backend. No server-side change is required.
...
export class SnapSmackGYSSAPI {
    ...
    async _call(method, endpoint, params = null, body = null) {
        ...
        // Route through the Python backend (CORS-free) instead of a browser fetch.
