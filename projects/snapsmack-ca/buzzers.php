    ...
    <section class="post-index">
        <div class="wrap">
            ...
            <ol>
                ...
                <li><span class="idx-date">Sep 2</span><a href="#a054">Desktop &harr; CMS &mdash; Can a Hole in One Half Be Used to Screw the Other?</a></li>
                <li><span class="idx-date">Sep 2</span><a href="#a053">The Shared Content Store &mdash; New Local Database, Same Standard</a></li>
    ...
    <section class="posts">
        <div class="wrap">
            ...
            <article class="post" id="a057">
                ...
                <p>What&rsquo;s left is not code. The executable carries no Microsoft-recognised publisher certificate, and it never will: we are not paying a third party every year so Windows will stop calling our software presumptively untrustworthy. The report says so plainly. SnapSmack releases are signed with our own key &mdash; the same Ed25519 signature the CMS updater and SMACKBACK already verify on every package &mdash; and that is the publisher identity we stand behind. Windows will show its warning on first run. That is Windows&rsquo; choice, not a hole in the software.</p>
            ...
            <article class="post" id="a054">
                <div class="post-meta"><span class="post-date">September 2, 2026 &mdash; closed September 18</span><span class="post-tag">Critical &amp; High Findings Closed</span></div>
                <h2>Desktop &harr; CMS &mdash; Can a Hole in One Half Be Used to Screw the Other?</h2>
                <p>The question was asked plainly: could a compromised blog push a miner or ransomware down to the desktop tools, and could a booby-trapped image run code in the photo editor? Four reviewers, six chokepoints, both directions. The server held: uploads are re-encoded (which strips hidden payloads), HTML goes through a real allowlist, skins are signed, and no server path can be pointed at the internal network. The desktop side had never had that kind of pass, and it showed. Three things mattered. A tampered profile file could quietly send a real posting key to an attacker&rsquo;s address, because the tool acted on whatever URL the file said &mdash; CRITICAL. The sorting tool (GYSS) ran its interface with no content-security policy and had a privileged file-write that could overwrite another tool&rsquo;s executable for the hub to launch later &mdash; the cleanest code-execution chain in the set. And server-supplied images reached native decoders with no size ceiling or format check.</p>
                <p>All of it is closed in code. Profile URLs must match the file they live in; discovery refuses to send the hub&rsquo;s key anywhere but a clean HTTPS origin, and validates every node the hub names. GYSS got a real content-security policy, its file-write is jailed and refuses executables, and the hub checks each tool&rsquo;s fingerprint before launching it. Decoding moved into a shared safe-open module and, in SNAP SLAPPER, into separate bounded worker processes. Every request that carries a key now refuses to follow redirects. One item was built and then deliberately dropped: signing backup packages. The owner&rsquo;s backup in the owner&rsquo;s cloud is theirs, not ours to vouch for. What stays is that our tool is careful with the owner&rsquo;s disk while unpacking one. Three items remain as decisions rather than defects &mdash; whether the credential vault should be mandatory, server-side enforcement of key scope, and a domain rule for discovered nodes &mdash; and they&rsquo;re listed in the report as exactly that.</p>
                <a class="report-link" href="secaudits/2026-09-02-054-desktop-cms-attack-surface-duty-of-care.pdf" target="_blank" rel="noopener">Read the full report &rarr;</a>
            </article>

            <article class="post" id="a053">
                <div class="post-meta"><span class="post-date">September 2, 2026 &mdash; closed September 18</span><span class="post-tag">No Critical or High &mdash; Closed</span></div>
                <h2>The Shared Content Store &mdash; New Local Database, Same Standard</h2>
                <p>The desktop tools grew a shared local database so the long-form editor can compose posts offline and record what it published. A new local database with file writes, new outbound calls, and cross-tool access to stored keys is new attack surface, so it got a full pass before it went near a live site. Result: clean. No Critical, no High. Three mediums and lows: the media store built filenames without containing them (an extension could escape or plant an arbitrary type) &mdash; fixed, filename jailed, image types allowlisted; full-EXIF originals with GPS were being copied into a shared, backup-eligible store &mdash; resolved by not copying at all (the tool records what it posted, it does not hoard the owner&rsquo;s files); and the sorter could report &ldquo;ok&rdquo; when resuming against a same-named different site &mdash; fixed to say not-found.</p>
                <p>The report is also honest about its own shortcomings: it reviewed a diff, and a diff review cannot find a hole that already existed in code the diff didn&rsquo;t touch. That&rsquo;s why 054 (above) followed the same day as a full-surface pass. The leftovers closed later: a silent failure in the library recorder now logs; a same-photo-in-two-posts case now keeps both; and the suite-wide rule that a request carrying a key never follows a redirect.</p>
                <a class="report-link" href="secaudits/2026-09-02-053-desktop-suite-audit-fixes-branch.pdf" target="_blank" rel="noopener">Read the full report &rarr;</a>
            </article>
            ...
            <article class="post" id="a052">
                ...
                <h2><code>llms.txt</code> &mdash; When Descriptive Data Looks Like Instructions</h2>
                <p><code>llms.txt</code> does not execute code. The risk starts when an outside AI agent reads machine-oriented documentation, mistakes untrusted text for instructions, and has permission to run commands. SnapSmack&rsquo;s generated file contained no installers, packages, fetch-and-run commands, or <code>llms-full.txt</code>, so this audit did not uncover a live command payload. It did find the boundary worth fixing before somebody had a reason to abuse it: the site name, description, and configured URL were inserted directly into agent-readable Markdown. A poisoned setting could manufacture headings, arbitrary links, role-labelled prompt text, or command-shaped prose for a badly sandboxed agent to follow.</p>
                <p>Release 0.7.575 makes those fields inert. Names and descriptions become single-line, length-bounded descriptive text; Markdown controls and publisher links are removed; descriptions containing prompt overrides, role injection, install commands, command substitution, or fetch-and-run syntax are omitted. Site URLs must be valid HTTP or HTTPS addresses with no credentials or control characters. Generated links are limited to the current site and hard-coded SnapSmack attribution, and SnapSmack continues not to publish the much larger <code>llms-full.txt</code> surface. Twenty-one regression checks pin the boundary. The detailed report also corrects an initially over-broad proposal: SMACKBACK protects the shipped generator, while each site&rsquo;s generated output is compared with that site&rsquo;s expected content rather than one impossible universal hash.</p>
                <a class="report-link" href="secaudits/2026-08-27-052-llmstxt-agent-execution-surface.pdf" target="_blank" rel="noopener">Read the full report &rarr;</a>
            </article>
            ...
            <article class="post" id="a050">
                ...
                <h2>CMS Architecture Assessment &mdash; Direct-Access Guards &amp; Content-Model Integrity</h2>
                <p>Where 047 and 048 reviewed the code line by line and against a live target, 050 was a structural review of the whole CMS against established publishing, media-library, database, recovery, security and federation patterns. It closed one security finding. Sensitive server-side includes &mdash; <code>updater.php</code>, <code>release-pubkey.php</code> (the release-signing public key), <code>skin-registry.php</code>, <code>manifest-inventory.php</code> and the real authentication include <code>auth-smack.php</code> &mdash; carried no PHP execution guard of their own; only <code>smackback.php</code> did. They relied on a single Apache <code>&lt;FilesMatch&gt;</code> deny-by-name list, which evaporates on nginx or any host ignoring <code>.htaccess</code>, leaving each include reachable as its own URL. The list had also drifted: it named dead <code>login</code>/<code>auth</code> entries and omitted <code>auth-smack.php</code>. Severity was medium (a trust-boundary/defence-in-depth gap, not RCE and not a direct leak &mdash; <code>db.php</code> does not echo). Release 0.7.544 adds a server-agnostic 403 direct-access guard to each include and reconciles the <code>&lt;FilesMatch&gt;</code> list to the files that actually ship. The code is released; per-installation verification via the updater/repair path is recorded as pending.</p>
                <p>A long-open data-integrity check for orphaned community records (comments, likes, reactions with no valid parent) was verified against the highest-risk site&rsquo;s database: zero true orphans (comments 3819&rarr;0, likes 4&rarr;0), 9,975 images reconciled against 9,975 posts. It is disclosed with the mistake made during it: an earlier diagnostic used a posts-only definition of &ldquo;orphan&rdquo;, which is wrong because engagement keys to the image identifier by design; that assumption mis-flagged ~1,904 valid comments, which were briefly removed and then restored from backup before the corrected, display-aware check ran clean.</p>
                <p>The review&rsquo;s foundational finding is architectural, and an earlier audit missed it while reporting the code clean: the CMS had no single enforced definition of a published item &mdash; legacy solo publishing stored a photograph as a bare image row rather than a post, so publication state, ownership, dates, engagement, collections and federation identity had no one home. That is a data-model question a code review does not surface; its remediation (post-model inventory and transactional conversion, 0.7.534D onward) is in progress, not finished. Of eight findings dispositioned, the two closed here are disclosed; the remainder &mdash; including further hardening of administrator authorization and URL routing &mdash; are deferred with owners and deliberately not mapped in public until they are fixed and can be disclosed the same way. The review process itself was corrected, not just the finding: our internal compliance standard now requires a comparative review to produce a cited platform-by-platform matrix and a deviation register &mdash; it cannot report &ldquo;clean&rdquo; from internal knowledge alone &mdash; and adds explicit data-model invariants, so a missing post object or an unbenchmarked architecture is a gated check a future review must clear.</p>
            ...
            <article class="post" id="a037">
                ...
                <p>One thing was left as-is on purpose: for plain FTPS, certificate checking stays off by default, because the budget shared hosts most photographers use ship broken or self-signed certificates and turning it on would simply break everyone's backups. That tradeoff is documented in the report, along with the fix &mdash; remembering your host's certificate on first use, the same trick used for SFTP, so you get tamper detection without needing your host to fix their certificate. Credentials closed in SUYB 0.7.19; the certificate memory shipped 2026-09-20 in SUYB 0.7.44 (see the 004 closure entry above for the renewal twist).</p>
            ...
            <article class="post" id="csrf">
                ...
                <p class="report-refs">Underlying reviews:
                    <a href="secaudits/2026-04-25-001-initial-full-codebase-audit.pdf" target="_blank" rel="noopener">001</a>,
