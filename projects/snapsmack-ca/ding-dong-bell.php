...
<main>
    ...
    <section class="posts">
        <div class="wrap">
            ...
            <div class="entry">
                <div class="entry-top">
                    <h3>One photograph, three places, three different comment counts</h3>
                    <span class="date">&ldquo;Count It Three Times&rdquo; &middot; 2026-09-14</span>
                    <span class="state notwatch">Fixed &mdash; confirming</span>
                </div>
                <p>The day after comments were called watched, the same photograph was opened in three places: four comments on the blog that owns it, two on Pixelfed, none in another SnapSmack blog&rsquo;s reader. Three separate causes, and the delivery queue was not one of them. The reader only knew how to ask a Pixelfed-style API for a thread, so every SnapSmack post answered with nothing. A comment typed before the outbound fix of 708D had never been sent. And a reply that came in from a third server was kept by the blog but never passed on to the blog&rsquo;s followers &mdash; the standard fediverse rule for that (inbox forwarding) was not implemented, and a SnapSmack blog on the receiving end would have rejected such a forward anyway.</p>
                <p>Built in 0.7.712D: every post now publishes a standard <code>replies</code> list any server can read, and the reader reads it; public replies to a post are forwarded to the post&rsquo;s followers, and forwarded activities are accepted only after being verified from where they were written; comments from before 708D are sent once, original dates kept, automatically and from a button. Regression-tested, not deployed, not yet watched on a live Pixelfed &mdash; and whether Pixelfed accepts a forwarded reply is the part only a live test settles. The doctrine: count it in three places before calling it working.</p>
                <p class="report-link-wrap"><a class="report-link" href="opaudits/2026-09-14-015-one-photograph-three-comment-counts.pdf" target="_blank" rel="noopener">Read the full report &rarr;</a></p>
            </div>
            ...
            <div class="entry">
                ...
                <p class="report-link-wrap"><a class="report-link" href="opaudits/2026-09-13-013-fediverse-replies-stored-never-shown.pdf" target="_blank" rel="noopener">Read the full report &rarr;</a></p>
            </div>
            ...
            <div class="entry">
                <div class="entry-top">
                    ...
                    <span class="date">&ldquo;Last One In&rdquo; &middot; 2026-09-14</span>
                    <span class="state watched">Watched working</span>
                </div>
                ...
                <p class="next-line"><strong>Update, same afternoon &mdash; why it was one site and not zero, and not thirty-seven:</strong> every site registers its jobs into the web server&rsquo;s crontab under a tag, and the tag is the same for every site. One crontab per box, one tag, room for one line: registering the next site silently replaces the last. That is why the fleet-wide re-registration in OPAUDIT 011 undid itself as it went, and why the last site through the door was the only one scheduled. The relay box had the same disease &mdash; its crontab held photofri.day&rsquo;s jobs and none of the relay&rsquo;s own; the relay had been thirteen hours stale and its curator frozen. The same box-level schedule was installed there at 16:26 UTC and watched: the relay&rsquo;s own page went from &ldquo;13h ago&rdquo; to &ldquo;3m ago&rdquo; on the next tick. Fleet board stayed green throughout because the page-hit fallback writes the very timestamp it reads. The code that let all of this happen is replaced in 0.7.712D (built, not yet deployed): crontab lines told apart by site, a SCHEDULER row on every Cron &amp; Jobs page that moves only when the scheduler itself launches the job, a fleet board that judges on that and can go red, and a post-deploy gate that goes red if a job has not fired within fifteen minutes of an update. The relay&rsquo;s hourly job was also watched firing on the new schedule at 17:07. <strong>Watched working, same evening:</strong> 714D deployed to every site, and the fleet board &mdash; now reading only the scheduler&rsquo;s own heartbeat &mdash; showed 25 of 25 sites green on all three jobs, hub included. Along the way the first true reading exposed three more faults that had been hiding behind the dead cron (a job that crashed on 18 sites every tick, a false post-deploy alarm, a hub with no key its own heartbeat would take); all three are in the same build. Full account in the report.</p>
                <p class="report-link-wrap"><a class="report-link" href="opaudits/2026-09-14-014-one-crontab-per-box-one-winner-per-job.pdf" target="_blank" rel="noopener">Read the full report &rarr;</a></p>
            </div>
            ...
            <div class="entry">
                ...
                <p class="next-line"><strong>Corrected 2026-09-14:</strong> the re-registration across the 24 spokes did not hold. Every site on a box writes the same tag into the same crontab, so each spoke&rsquo;s registration replaced the one before it; one site ended up scheduled. See &ldquo;Last One In&rdquo; above (OPAUDIT 014).</p>
