...
$faq_qas = <<<'HTML'
            ...
            <div class="qa" id="q-install-modes">
                ...
                <p>SnapSmack ships in four install personalities — SMACKONEOUT (single-user photoblog), GRAMOFSMACK (a faithful 2016-Instagram-clone with three-across grids and carousel posts up to ten images deep), SMACKTALK (essays and pages alongside the photoblog), and SMACKTHEMUP (Picasa-style public albums for friends and family, managed from SNAP SLAPPER, no fediverse). You pick one when you install. You don't switch later.</p>
                <p>This is deliberate. Each mode has its own database conventions, its own admin behaviours, its own assumptions about what a post is, its own visible feature set. Letting installs toggle between them would mean every feature has to handle four modes plus every transition state between them. That is the road to bloat and to the kind of bugs that don't get found until somebody loses data.</p>
