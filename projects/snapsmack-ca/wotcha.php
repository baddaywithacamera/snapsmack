    ...
    <section class="posts">
        <div class="wrap">
            ...
            <article class="post" id="slapper-file-format">
                ...
                <p>That is the point of the <code>.slapper</code> project file. Underneath the rude little extension, it is an <strong>ordinary ZIP archive</strong>. Rename <code>something.slapper</code> to <code>something.zip</code> and open it with the ZIP tool already on your computer. Inside is a plain-text README and a versioned, human-readable <code>project.json</code> describing which original photograph the project belongs to, the crop and geometry, global adjustments, layers, masks, blend modes, retouch work, Filters, and the LEWKS stacked on top.</p>
                <p>The photograph stays where the photographer put it. SNAP SLAPPER stores the instructions and standard project assets in the open container, not a secret second original buried in an app dungeon with a tiny proprietary door.</p>
                <p>The format is deliberately boring where boring is beautiful. ZIP. Structured data. Declared versions. Ordinary paths. Standard image assets. No executable scripts hiding in a style pack, no cloud account required to inspect the file, and no mystery database that turns into alphabet soup the minute the company gets acquired by Bastard Holdings LLC.</p>
                ...
                <p>That also means a <code>.slapper</code> file is small enough to back up, inspect, copy, unpack, archive, and hand to another machine. The extension identifies the job; it does not disguise the construction. SUYB&rsquo;s SLAP HAPPY backup knows where the projects, settings, originals, and saved photographs live because the photographer declares those locations. Full and incremental backups preserve the pieces without dragging the entire library into SNAP SLAPPER&rsquo;s program folder. Your filing system remains your filing system. Revolutionary stuff, apparently.</p>
            ...
            <article class="post" id="pixelix-live">
                ...
                <p>Native phone posting is live first on GRAMOFSMACK. <a href="https://github.com/Daniebeler/pixelix" target="_blank" rel="noopener"><strong>Pixelix</strong></a> is a native Pixelfed client for publishing photographs and participating in the photo-centric Fediverse. It can connect directly, register through OAuth, pass through the owner&rsquo;s normal login and two-factor authentication, upload a photograph as a private CMS draft, attach ALT text, and publish the finished post. The first real-device test on ALLINTHEWRIST completed that entire path and the post appeared in both SNAPSMACK and Pixelix.</p>
            ...
            <article class="post" id="posse">
                ...
                <p>We're drawing the line at one network, on purpose: the <strong>Fediverse</strong> — ActivityPub, Pixelfed, Mastodon, and anything else that speaks the protocol. Not because it's the biggest room, but because it's the only one where the ethos is baked into the wiring itself — open, self-hostable, federated, owned by nobody. Every other network is somebody's property wearing a friendly face, and syndicating into a silo to chase reach is just handing your work back to a landlord through a different door. So we don't. No Threads, no Bluesky, no chasing whatever launches next quarter, no reach for reach's sake. One open protocol, chosen on principle, door closed.</p>
                <p>Here's the part the name is honest about. POSSE is not for pussies. It asks the one thing the free silos never do: that you run your own patch of ground. Here's the part that surprises people, though — that doesn't take expensive iron. SnapSmack is light; it doesn't hammer a server the way WordPress does, so it runs happily on ordinary, modest hosting. What it asks isn't horsepower, it's ownership — that you take responsibility for your own site instead of renting a room in someone else's tower. The reward is that nobody can deplatform you, rug-pull you, train a model on your catalogue, or quietly turn a decade of your work into engagement bait. The bill is that it's yours to run. If that trade sounds like too much, the silos are right over there, waiting to own you. If it sounds like freedom, you already knew that before you finished reading this.</p>
                <p>Pixelfed support is live now — SMACKONEOUT and GRAMOFSMACK can speak the Fediverse the moment you want them to. Flip the switch and your posts syndicate out to the whole party; leave it off and your blog stays a quiet night in, lights low and door locked, with anyone who wants you coming to your place on your terms. Federating is a switch you flip — never a default we flip for you.</p>
            </article>
            ...
            <article class="post" id="feds-at-door">
                ...
                <p>Here's the surprise: it barely costs you anything to run. Federation is just HTTP going back and forth — signed messages out, signed messages in — so it doesn't stress the box or demand a beefy server. No heavy daemon, no queue chewing your CPU, no reason it can't live on the same ordinary hosting your photoblog already sits on. The Fediverse makes the old new again, without making your server sweat.</p>
                <p>Live now, and entirely your call — SMACKONEOUT and GRAMOFSMACK can present to the Fediverse straight from your own server the moment you switch it on, and stay dark until you do.</p>
            </article>
            ...
            <article class="post" id="skin-drop">
                ...
                <p>AURORA and PARADE ship alongside it. AURORA is the quiet one: a slow wash of pastel light behind the grid, animated nav lines, and a border that drifts rather than blinks. PARADE is the LGBT+ identity skin: a full-screen waving flag behind the photographs with coordinated flag-coloured tile borders. Choose Rainbow, Progress Pride, Trans, Bisexual, Non-Binary, Pansexual, Lesbian, Asexual, Aromantic, Genderfluid, Genderqueer, or Two-Spirit. Wave speed, depth, opacity, nav-line opacity, and spacing are adjustable, and reduced-motion visitors receive a still flag.</p>
            ...
            <article class="post" id="break-glass">
                ...
                <p>It's called Break the Glass. You generate a small recovery file ahead of time and keep it somewhere safe and offline — an encrypted USB stick, a password manager — well away from the site it belongs to. If you're ever completely locked out, you upload that one file to your site, visit the recovery page, verify the account it names, and burn it. SnapSmack revokes the old password, authenticator, recovery codes, trusted devices, reset links, sessions, and the card itself before making you establish everything again. No email round-trip, no third party, no begging a platform to believe you're really you.</p>
                ...
                <p>But understand what that axe is: a one-use master key, and it is yours to guard. The real card works — that's the entire point of it — which means whoever holds the genuine file can get into your site, exactly the way your house key works for anyone holding it. Making a replacement kills the previous card; using one kills it before access is restored. So keep it offline and out of sight, treat it like the spare key to everything you own, and never write down or mention which site it unlocks. Naming the site turns a lost file into an instant break-in. Forgeries fail; the real one doesn't — so the real one stays secret.</p>
