...
index) is specced in `_spec/photoblogs-relay-node-spec-v1.md`.
...
1. Create the database and (optionally) load the schema — the app also
   ...
   ```
   mysql -e "CREATE DATABASE photoblogs_relay CHARACTER SET utf8mb4"
...
4. First hit to `/actor` generates the relay keypair and creates tables. Verify:
   `curl https://photoblogs.fyi/actor` returns the `Application` actor.
5. Add the drain cron (also drains inline after each POST):
   ```
   * * * * * /usr/bin/php /srv/photoblogs-relay/cron/drain.php >> /var/log/relay-drain.log 2>&1
   ```

## Operate

- Console: `https://photoblogs.fyi/admin.php?token=YOUR_ADMIN_TOKEN`.
