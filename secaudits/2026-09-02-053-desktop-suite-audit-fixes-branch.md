...
- [x] D - reused-image join table — FIXED 2026-09-05 in 651D (`snap_library.py`: `post_assets(post_id, asset_id, ord)`; `assets_for()` reads the join table; legacy `assets.post_id` folded in and consumed on every open). Sean 2026-09-18: the local mirror MUST match the online DB — a missing post is a bug, not a preference. Verified today: same asset recorded for posts 1 and 2 → both return it. Regression `tools/_shared/tests/test_snap_library_reuse.py`
...
D had already been fixed on Sep 5 (651D) — this report was stale on that too, and
my first closure pass repeated the stale text. Sean's rule, recorded: the local
mirror must match the online DB; a post missing from it is breakage. Audit CLOSED;
only the live-post acceptance check remains, and that is feature acceptance.
