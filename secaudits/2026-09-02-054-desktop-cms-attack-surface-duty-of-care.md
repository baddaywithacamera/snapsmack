...
- [x] Backup archive sign + verify-before-restore — SUYB 0.7.44 `backup_signing.py`: HMAC-SHA256 over per-member sha256 map, local-only key; altered/added/foreign packages refused, pre-0.7.44 unsigned packages ask (headless refuses). Test: `tests/test_backup_signing.py` (12)
...
0.7.11 rebuild); one watched SUYB backup → restore round-trip on 0.7.44 to see
