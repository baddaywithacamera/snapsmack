/**
 * SNAPSMACK — JIVE TURKEY tile-border engine (Layer 2): INSIDE COLOUR RING
 *
 ...
 * ring drawn INSIDE the tile on its edge — the .jt-ring overlay, masked to a
 * band of --tile-bw. The photo is inset by that same width (skin CSS) so the
 * ring frames the image without ever covering it and no image area is lost.
 * The colourway colour walks across the grid in the chosen direction — a wave
 * of colour, smoothed by a CSS transition on the ring background.
 *
 * Because the ring lives INSIDE each tile, the full Tile Spacing (--grid-gap)
 * stays as pure background between tiles — the border never grows into the
 * gutter. This SUPERSEDES the OUTWARD box-shadow band of 0.7.425, which grew
 * into the gutter (eating the tile spacing) and revealed the tile background as
 * a second line once the border colour was light. Keeps the reliable-paint +
 * hidden-tab fallback + tile-count re-scan so lazy-loaded / infinite-scroll
 * tiles still get their border.
 *
 * The band WIDTH is constant (--tile-bw, from the skin); the COLOUR is what
 * animates. AURORA/PARADE use a DIFFERENT border engine; this one is JIVE
 * TURKEY only.
 *
...
(function(){
  ...
  function scan(){
    ...
    COLS=liveCols(COLS);
    // INSIDE ring: clear any stale outward box-shadow the old engine may have left.
    // The ring band width is the skin's --tile-bw; this engine drives its COLOUR only
    // and NEVER writes any layout property (gap/padding), so the 935px column and the
    // Tile Spacing are owned entirely by CSS. Ring visibility is CSS's job too
    // (phantom tiles keep their ring hidden via the skin stylesheet).
  ...
  // INSIDE ring: paint the masked .jt-ring band a solid colourway colour. Constant
  // width (from CSS --tile-bw); the CSS transition on .jt-ring smooths colour flips.
  function paint(tile,col){ var r=ringOf(tile); if(r) r.style.background = col||'transparent'; }
  ...
  function paintAll(now){
    ...
    if(!S.enabled){ for(var j=0;j<S.tiles.length;j++){ var rj=ringOf(S.tiles[j]); if(rj) rj.style.background='transparent'; } return; }
    if(reduced){ for(var k=0;k<S.tiles.length;k++) paint(S.tiles[k],S.COLS[0]); return; }
    // Colour cadence: seconds per colour step, eased by Speed. Wave offset walks the
    // colour across the grid in the chosen direction (Wave spreads the offset).
    var t=now/1000, D=0.8+Math.pow((100-S.SPD)/100,2)*18, waveAmt=(S.WAVE/100)*0.9;
    for(var i=0;i<S.tiles.length;i++){
      var g=S.geo[i]||{row:0,col:0}, off=ord(S.DIR,g.row,g.col,S.rows,S.cols)*waveAmt, local=t/D+off, step=Math.floor(local);
      paint(S.tiles[i],S.COLS[((step%S.COLS.length)+S.COLS.length)%S.COLS.length]);
    }
  }
