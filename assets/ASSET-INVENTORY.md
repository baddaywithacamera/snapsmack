<!-- SNAPSMACK_EOF_HEADER: last non-empty line must be the canonical HTML EOF marker. -->
...
- **`smack-prompt-schedule.js`** — Schedule a Prompt (admin helper) Two small conveniences on the PHOTO CHALLENGE admin's SCHEDULE A PROMPT panel: 1.
...
- **`ss-engine-game-on.js`** — GAME ON sliding-puzzle engine
...
- **`photochallenge-board-embed.css`** — Photo-Challenge board CARD appearance (0.7.621D) Scoped under .pc-board so it never touches the host page's theme.
- **`photochallenge-board-layouts.css`** — PHOTO CHALLENGE public feed layouts Layout choices for /board.
