...
/*
 * skin_registry_install_upload() removed 2026-08-09 (security).
 *
 * It accepted an owner-uploaded skin ZIP and unpacked executable PHP into
 * SKINS_DIR. Receiving and unzipping an untrusted archive on the production
 * server is itself attack surface (zip-parser / upload-handling zero-days), so
 * the capability was deleted rather than gated — a gate is still a door. Skins
 * now enter only through git, examined locally by the owner, then shipped via
 * the signed registry. Registry installs still verify a detached signature; see
 * skin_registry_install() above.
 */
