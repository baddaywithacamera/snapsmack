...
<?php if (!$filtered): ?><p class="pbf-directory-empty"><?php echo !$rows?'No blogs are listed yet.':'No blogs match that search.'; ?></p><?php else: ?><div class="pbf-directory-table-wrap"><table class="pbf-directory-table"><thead><tr><th scope="col">Name</th><th scope="col">Topics</th><th scope="col">Posts</th><th scope="col">Last updated</th></tr></thead><tbody><?php foreach ($filtered as $row): ?><?php $activity=$row['last_post_at']??$row['updated_at']??''; ?><tr><th scope="row"><a href="<?php echo $h($row['site_url']); ?>" rel="noopener"><?php echo $h($row['name']); ?></a><small><?php echo $h($row['host']); ?></small></th><td><?php if ($row['_topics']): ?><?php foreach (array_slice($row['_topics'],0,6) as $i=>$name): ?><?php if ($i): ?> &middot; <?php endif; ?><a href="<?php echo $url(['topic'=>$name,'sort'=>$sort]); ?>"><?php echo $h($name); ?></a><?php endforeach; ?><?php else: ?>&mdash;<?php endif; ?></td><td class="pbf-directory-number"><?php echo isset($row['post_count']) ? number_format((int)$row['post_count']) : '&mdash;'; ?></td><td><?php echo $activity!=='' ? $h(date('Y-m-d',strtotime((string)$activity))) : '&mdash;'; ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
