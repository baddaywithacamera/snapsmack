...
$forum_mod_key = $settings['forum_mod_key']  ?? '';
...
if ($action === 'save-mod-key') {
    $key = trim($_POST['mod_key'] ?? '');
    if ($key !== '') {
        $pdo->prepare(
            "INSERT INTO snap_settings (setting_key, setting_val) VALUES ('forum_mod_key', ?)
             ON DUPLICATE KEY UPDATE setting_val = ?"
        )->execute([$key, $key]);
        $forum_mod_key = $key;
        $settings['forum_mod_key'] = $key;
    }
    header('Location: smack-forum.php');
    exit;
}

if ($action === 'add-category' && $forum_mod_key !== '') {
    $cat_name = trim($_POST['cat_name']        ?? '');
    $cat_slug = trim($_POST['cat_slug']        ?? '');
    $cat_desc = trim($_POST['cat_description'] ?? '');
    $cat_sort = (int)($_POST['cat_sort']       ?? 0);

    if ($cat_name !== '') {
        $res = forum_api('POST', 'categories', [
            'name'        => $cat_name,
            'slug'        => $cat_slug,
        ...
        if (!$res['_error']) {
            header('Location: smack-forum.php?board_added=1');
            exit;
        }
        $msg = 'Error: ' . ($res['message'] ?? 'Could not create board.');
    } else {
...
$cat_accents = ['#888888','#e2b714','#39FF14','#00bcd4','#ab47bc','#ff7043','#26a69a','#5c6bc0'];
      ...
      <div class="box-body">
        ...
        <form method="post" action="smack-forum.php">
          ...
          <div class="lens-input-wrapper">
            ...
            <input type="text" name="cat_name" maxlength="100" placeholder="General Chat" required
                   value="<?php echo htmlspecialchars($_POST['cat_name'] ?? ''); ?>">
          </div>
          <div class="lens-input-wrapper">
            <label>SLUG <span class="field-tip" data-tip="Leave blank to auto-generate from the name.">ⓘ</span></label>
            <input type="text" name="cat_slug" maxlength="50" placeholder="general-chat"
                   value="<?php echo htmlspecialchars($_POST['cat_slug'] ?? ''); ?>">
          </div>
          <div class="lens-input-wrapper">
            <label>DESCRIPTION <span class="field-tip" data-tip="Optional.">ⓘ</span></label>
            <input type="text" name="cat_description" maxlength="255" placeholder="Optional — leave blank if not needed"
                   value="<?php echo htmlspecialchars($_POST['cat_description'] ?? ''); ?>">
          </div>
          <div class="lens-input-wrapper">
            <label>SORT ORDER <span class="field-tip" data-tip="Lower numbers appear first. Default 0.">ⓘ</span></label>
            <input type="number" name="cat_sort" min="0" value="<?php echo (int)($_POST['cat_sort'] ?? 0); ?>" style="width:100px;">
          </div>
          <div style="display:flex; gap:12px; justify-content:flex-end; margin-top:16px;">
            <a href="smack-forum.php" class="action-edit" style="line-height:36px;">CANCEL</a>
            <button type="submit" class="forum-new-btn">Create Board</button>
          </div>
        </form>
        <?php endif; ?>
      </div>
